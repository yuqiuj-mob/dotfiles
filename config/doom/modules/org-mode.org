...
* Auto Tangle
Automatically tangle org files on save, so =config.el= stays in sync.
...
* Org Modern
Clean modern styling for org buffers: styled headlines, keywords, tables.
#+begin_src emacs-lisp
...
** Indent Bars
=org-modern-indent= draws org-modern-style bars along indented content
(replaces org-indent visuals). Org body text stays in the default fixed-pitch
font, consistent with markdown and code buffers.

Mixed-pitch (proportional prose font) is intentionally left off — re-enable by
adding =(use-package! mixed-pitch :hook (org-mode . mixed-pitch-mode))= here.
#+begin_src emacs-lisp
...
* Export
** LaTeX
=cleveref= support for cross-references.
#+begin_src emacs-lisp
...
Execute WaveDrom JSON blocks to produce SVG waveform diagrams.
#+begin_src emacs-lisp
...
** Assembly (NASM)
Compile and execute x86-64 assembly blocks.
#+begin_src emacs-lisp
...
* Templates
Org-roam capture templates for notes, meetings, design docs, specs, and buglogs.
#+begin_src emacs-lisp
...
* Bibliography
Org-roam-bibtex integration with citar for reference management.
#+begin_src emacs-lisp
...
* Websocket
Required for org-roam-ui (if enabled in the future).
#+begin_src emacs-lisp
