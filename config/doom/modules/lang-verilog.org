...
imenu is searchable by module name but not instance name. Relabel instances so
both are searchable, laid out as two columns: the instance name (in a distinct
color) followed by the module type (dimmed).

The instance column width is computed per index build as the longest instance
name in the buffer (capped), so the type column stays aligned even for long
names, always with a gap.

Scoped to imenu only via a dynamic flag bound around index creation, so the
=which-func= breadcrumb (which shares the same name function) keeps its stock
behavior. The label formatter is swapped for one that preserves text
properties, since the default =(format "%s" name)= would strip the faces.
#+begin_src emacs-lisp
