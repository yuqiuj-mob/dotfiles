...
imenu is searchable by module name but not instance name. Relabel instances as
=instance_name:module_type= so both are searchable.

Scoped to imenu only via a dynamic flag bound around index creation, so the
=which-func= breadcrumb (which shares the same name function) is left at its
stock behavior.
#+begin_src emacs-lisp
